...
import '../models/response/ubication_one_response.dart';
...
class PlotServices {
  ...
  Future<ResponseDefault> registerParcel({
    ...
    try {
      ...
      final uri = Uri.parse('${Environment.endpointApiPlots}/register-parcel');
  ...
  //Obtener Ubicacion de una Parcela
  Future<UbicationOneResponse> getUbicationOne() async {
    ...
    final resp = await http.get(Uri.parse('${Environment.endpointApiPlots}/get-ubication-one?id_user=$userId'),
      headers: {
      'Accept': 'aplication/json',
        'xx-token': refreshToken!
      }
    );

    if (resp.statusCode == 200) {
      return UbicationOneResponse.fromJson(jsonDecode(resp.body));
    } else {
      throw Exception('Failed to load plot data');
    }
  }
  
  // Agrega este método a tu PlotServices
  Future<ResponseDefault> updateParcel({
    required String name,
    required String ubicacion,
  }) async {
    try {
      ...
      if (token == null || userId == null) {
        return ResponseDefault(resp: false, msg: 'Authentication required');
      }

      final uri = Uri.parse('${Environment.endpointApiPlots}/update-parcel');
      ...
      if (response.statusCode == 200) {
        ...
        return ResponseDefault(
          resp: false,
          msg: errorData['msg'] ?? 'Failed to update plot',
        );
      }
    } catch (e) {
      return ResponseDefault(
